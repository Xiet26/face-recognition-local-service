package utilities

const (
	TIME_FORMAT              = "2006-01-02 15:04:05"
	HOUR_FORMAT              = "15:04:05"
	BIRTH_FORMAT             = "02/01/2006"
	BIRTH_FORMAT_ATTEND      = "02-01-2006"
	RangeAbsent              = 20
	ImagePersonFolderPath    = `%s/data/images/person/%v`
	ImageBatchFolderPath     = `%s/data/images/batch/%s/%d/%s`
	ImageBatchFolderPathAll  = `%s/data/images/batch/%s/%d/%s/all`
	ImageBatchFolderPathFace = `%s/data/images/batch/%s/%d/%s/face`
	ImageLicenceFolderPath   = `%s/data/images`
	DataLicenceFolderPath    = `%s/data/`
	ImageFacePath            = `%s/%v_face.jpg`
	ImageBatchPath           = `%s/%v_face_%v_%v.jpg`
	ImageBatchAllPath        = `%s/%v.jpg`
	ModelPath                = "../models"
	PersonLabelFace          = `%s_%s`
	FileFieldXLSX            = "fileUpload"
	FileFieldFaceImage       = "faceImage"
	FaceIDField              = "faceID"
	PersonSheet              = "person"
	RoomSheet                = "room"
	BatchSheet               = "batch"
	SubjectSheet             = "subject"
	NumOfFrame               = 50
)
